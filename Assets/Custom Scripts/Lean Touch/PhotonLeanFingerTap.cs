...
namespace Lean.Touch
{
	...
	public class PhotonLeanFingerTap : Photon.MonoBehaviour
	{
        public GameObject swappableMolecule;
		...
		private void FingerTap(LeanFinger finger)
		{
            ...
            //Swap molecules and call RPC method OnObjectChanged when double tapped
            if(RequiredTapCount == 2)
            {
                    Debug.Log("current: " + currentMolecule.gameObject.GetPhotonView().name);
                    Debug.Log("swapped: " + swappableMolecule.gameObject.GetPhotonView().name);
                    photonView.RPC("OnObjectChanged", PhotonTargets.AllBufferedViaServer, currentMolecule.gameObject.GetPhotonView().viewID, swappableMolecule.gameObject.GetPhotonView().viewID);
                    int tmp = currentMolecule.gameObject.GetPhotonView().viewID;
                    currentMolecule.gameObject.GetPhotonView().viewID = swappableMolecule.gameObject.GetPhotonView().viewID;
                    swappableMolecule.gameObject.GetPhotonView().viewID = tmp;
